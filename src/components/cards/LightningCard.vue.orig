...
export default {
    ...
    watch: {
        ...
        'shared.localConfig': {
            handler(newConfig) {
                if (!newConfig) return;
                console.log("Global config change detected. Updating properties...");

                // Instead of replacing the object, update the keys inside it.
                // This forces Vue to see the "deep" change.
                Object.assign(this.localConfig, JSON.parse(JSON.stringify(newConfig)));

                // If you have specific UI toggles or filters, trigger them now
    ...
    methods: {
        ...
        processIncomingStrike(data) {
            ...
            if (dist <= tthis.localConfig.searchRadius) {
        ...
        saveSettings() {
            // 1. Ensure we are talking to the reactive global state
            const globalConfig = this.shared.localConfig;

            // 2. Update properties (using Number() to ensure they stay as digits)
            globalConfig.homeLocation.lat = Number(this.localConfig.homeLocation.lat);
            globalConfig.homeLocation.lon = Number(this.localConfig.homeLocation.lon);

            globalConfig.searchRadius = Number(this.localConfig.searchRadius);
            globalConfig.alertThreshold = Number(this.localConfig.alertThreshold);
            globalConfig.sampleSize = Number(this.localConfig.sampleSize);
            globalConfig.sensitivity = Number(this.localConfig.sensitivity);

            // 3. Persistence - use the same key as your old code if that's what you prefer
            localStorage.setItem('lightning_config', JSON.stringify(globalConfig));
            ...
            console.log("Global settings synced and saved.");

            // 4. THE UI FIX: Toggle both just to be safe, 
            // or check your template for which one controls the v-dialog
